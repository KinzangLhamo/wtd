...
def admin_login_page():
    ...
    def login():
        global is_admin
        # Update the admin credentials and password here
        if email_entry.get() == "LRC" and password_entry.get() == "admin_password":
...
def inventory_status_page():
    ...
    tk.Button(root, text="Price", command=lambda: show_price_details("BEPE")).pack(pady=5)  # Provide a default department
...
def update_inventory_page():
    ...
    def update_item():
        ...
        if item_name in inventory_data and new_price.replace('.', '', 1).isdigit():
            ...
            conn.commit()
            conn.close()
            load_inventory_data()  # Reload inventory data after update
