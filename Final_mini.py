...
def department_list_page():
    ...
    for dept in departments:
        tk.Button(
            root, 
            text=dept,
            command=lambda d=dept: raw_materials_options_page(d)
        ).pack(pady=10)

        tk.Button(
            root, 
            text=f"Show Price List for {dept}",
            command=lambda d=dept: show_price_details(d)  # Correctly passing the dept argument
        ).pack(pady=5)
